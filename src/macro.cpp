...
LispValue renameVarsRec(const LispValue& value, RenameMap& renamed, const LispValue& env, LispMap& map) {
    if (value.type == LispValue::Atom
            and value.atom() != "..."
            and value.atom() != "_") {
        ...
        if (rename != renamed.end()) {
            ...
            for (auto pair : map) {
//                auto id = findIdentifiers(pair.second);
                if ((pair.second.type == LispValue::Atom
                        and pair.second.atom() == value.atom())
                        /*or std::find(id.begin(), id.end(), value.atom()) != id.end()*/) {
                    auto new_name = genName();
                    renamed.emplace(value.atom(), new_name);
                    auto res = map.find(value.atom());
                    if (res != map.end()) {
                        map.emplace(new_name, res->second);
                        map.erase(res);
                    }
        ...
        if (value.list().front().type == LispValue::Atom) {
            auto& atom = value.list().front().atom();
            if (atom == "quote")
                return value;
            else if (atom == "lambdaSF@YALI" or atom == "defineSF@YALI") {
                auto car1 = value.list().front();
                auto car2 = value.list().rest().list().front();
                auto cdr = renameVarsRec(value.list().rest().list().rest(), renamed, env, map);
                return PairList(car1, PairList(car2, cdr));
            }
        }
        auto car = renameVarsRec(value.list().front(), renamed, env, map);
        auto cdr = renameVarsRec(value.list().rest(), renamed, env, map);
        return PairList(car, cdr);
    } else {
        return value;
    }
...
LispValue renameVars(const LispValue& value, const LispValue& env, LispMap& map) {
    RenameMap rename;
    return renameVarsRec(value, rename, env, map);
}

LispValue valueSubtitution(LispMap& map, const LispValue& value, const LispValue& env) {
    LispValue new_value;
    new_value = renameVars(value, env, map);
    return valueSubtitutionRec(map, new_value);
}

LispValue syntaxRules(const LispValue& form, const LispValue& env) {
    auto argsList = form.list().rest();
    if (argsList.list().front().type != LispValue::List) throw badSpecialForm(form);
    LispVector keywords;
    if (!argsList.list().front().isNull()) {
        for (auto key : argsList.list().front().list()) {
            if (key.type != LispValue::Atom) throw badSpecialForm(form);
            keywords.push_back(key);
        }
    }
    argsList = argsList.list().rest();
    if (argsList.isNull()) throw badSpecialForm(form);
    ...
    for (auto pair : argsList.list()) {
        if (pair.type != LispValue::List or pair.list().size() != 2
                or pair.list().front().type != LispValue::List) throw badSpecialForm(form);
    ...
    auto func = [keywords, matchVector, env] (auto args) {
        ...
        for (auto pair : matchVector) {
            ...
            if (p) {
                return valueSubtitution(keymap, env, pair.second);
            }
        }
...
LispValue syntaxCase(const LispValue& form, const LispValue& env) {
    ...
    argsList = argsList.list().rest();
    if (argsList.isNull()) throw badSpecialForm(form);
    std::list<std::tuple<LispValue, LispValue, LispValue>> matchVector;
    LispValue guard;
    for (auto pair : argsList.list()) {
        ...
        if (size == 2)
            ...
            matchVector.push_back({pattern, *std::next(pair.list().begin()), value});
        }
    }
    bool p;
    LispMap keymap;
    ...
    for (auto tuple : matchVector) {
        ...
        if (p) {
            bool g = valueSubtitution(keymap, guard, env).eval(env).isTrue();
            if (g)
                return valueSubtitution(keymap, value, env);
            else
                patternError(expr);
        }
    }
    ...
    if ((val.type == LispValue::List and !val.isNull()
        ...
        if (val.type == LispValue::List) {
           /* if (val.list().front().atom() == "begin") {
            ...
            } else*/ if (val.list().front().atom() == "syntax-case") {
                return {val, false};
            }
        }
        LispValue macro;
